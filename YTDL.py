            ...
            if u_show_latest_video_date:
                ...
                try:
                    ...
                    for i in range(len(latest_video)):
                        ...
                        print(print_colored_text(f"\r" + " " * (len(str(u_index)) + 2) + "Checking video " + print_colored_text(latest_video[i].video_id
                                + ": " + str(i + 1) + " of " + str(len(latest_video)), BCOLORS.CYAN) + print_colored_text(" from " + latest_video[i].author, BCOLORS.DARK_CYAN), end="", flush=True)
                        if (latest_video[i].vid_info.get('playabilityStatus', {}).get('status') != 'UNPLAYABLE' and
